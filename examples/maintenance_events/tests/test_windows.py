...
def test_days_since_last_event_ignores_the_future():
    ...
    hit = [w for w in windows if w.as_of == pd.Timestamp("2024-02-15")][0]
...
def test_label_coverage_is_the_span_of_the_chosen_source():
    """Cada registro cubre un período distinto: mapro 2022-08 a 2024-07, el log
    de operarios 2023-11 a 2024-12. Fuera de su período, 'no hay evento' significa
    'no hay registro', no 'no pasó nada'."""
...
def test_windows_outside_the_label_coverage_are_dropped():
    """Sin esto, elegir --label-source operator agrega mil ventanas etiquetadas
    negativas por falta de registro, y la campaña compara fuentes sobre
    cantidades distintas de basura."""
