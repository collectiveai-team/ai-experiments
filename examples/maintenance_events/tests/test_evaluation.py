...
def test_fold_without_positives_is_reported_as_none_and_excluded():
    ...
    folds, summary, _ = evaluate(X, y, as_of, lambda: DummyClassifier(strategy="prior"), n_folds=3)
...
def test_summary_reports_the_baseline_next_to_the_objective():
    ...
    _, summary, _ = evaluate(X, y, as_of, lambda: DummyClassifier(strategy="prior"), n_folds=3)
...
def test_every_tested_window_keeps_its_prediction():
    """El estimador nuevo puntúa una sola vez sobre todas las predicciones out-of-fold.

    Promediar el PR-AUC de cada bloque tira la mitad de la información: son medias de muestras
    chicas, no una medición.
    """
    X, y, as_of = _scored()
    folds, _, oof = evaluate(X, y, as_of, lambda: DummyClassifier(strategy="stratified"), n_folds=4)
...
def test_a_replicate_pairs_its_score_with_its_own_base_rate():
    """La tasa base se mide dentro de la réplica.

    Tomarla de la muestra original daría un lift que ninguna réplica alcanzó — el mismo error que
    `baseline_metric` evita entre folds.
    """
    X, y, as_of = _scored()
    _, _, oof = evaluate(X, y, as_of, lambda: DummyClassifier(strategy="stratified"), n_folds=4)
...
def test_overlapping_windows_widen_the_interval():
    ...
    def se(block):
        lifts = [
            pr - base for pr, base in block_bootstrap(oof, n_resamples=300, block=block, seed=3)
        ]
...
def test_a_block_draw_keeps_its_neighbours():
    """Un bloque es un tramo contiguo.

    Eso es lo que conserva la correlación que el remuestreo de a una destruye.
    """
...
def test_a_replicate_without_both_classes_is_dropped_not_scored():
    """Un PR-AUC sobre una réplica sin positivas no está definido.

    Mandar un cero sería una observación que baja el promedio sin haber medido nada.
    """
    as_of = _as_of(120)
