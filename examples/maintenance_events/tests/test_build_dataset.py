...
def test_build_never_leaks_operator_names(tmp_path):
    ...
    assert set(events.columns) == {
        "date", "source", "duration_hours", "buckets_left", "buckets_right", "scheduled",
    }
...
def test_build_is_deterministic(tmp_path):
    ...
    first = json.loads((build(raw_data, tmp_path / "a").parent / "maintenance-events-v1" / "manifest.json").read_text())
    second = json.loads((build(raw_data, tmp_path / "b").parent / "maintenance-events-v1" / "manifest.json").read_text())
