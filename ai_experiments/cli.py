...
def campaign_suggest(
    ...
    _require_campaign(FilesystemRunStore(runs_dir), campaign_id)
    try:
        trial = _orchestrator(runs_dir).suggest(
            campaign_id, parsed, note=note, variant_id=variant
        )
    except ValueError as exc:
...
def campaign_variant(
    ...
    parent: Optional[str] = typer.Option(
        None, "--parent", help="The variant this one builds on"
    ),
    runs_dir: Optional[Path] = typer.Option(None, "--runs-dir"),
    output_json: bool = typer.Option(False, "--json"),
) -> None:
    ...
    Whole files on disk, not text on a command line: what runs is exactly what
    was written, and it stays readable afterwards. The exit code is the smoke
    check's verdict — 2 means the variant does not start and was discarded.
    """
    ...
    if output_json:
        ...
        typer.echo(
            f"{record.variant_id}  {verdict}  edited {', '.join(record.edited_paths)}"
        )
        typer.echo(
            f"  iax campaign suggest {campaign_id} --params '{{...}}' "
...
def campaign_variants(
    ...
    for record in records:
        verdict = {True: "smoke ok", False: "rejected", None: "unchecked"}[
            record.smoke_ok
        ]
        if record.discard_error:
