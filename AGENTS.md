...
- The house standards this repo is held to are the CES catalog under
  [Engineering Standards](#engineering-standards) below, with the detail files in
  `.agents/rules/`.
...
Issues live as GitHub issues in `collectiveai-team/ai-experiments`, driven through the `gh` CLI.
