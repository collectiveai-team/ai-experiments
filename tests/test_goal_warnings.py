...
def test_a_goal_that_never_says_what_success_means_is_flagged():
    """Warn when nothing written down says whether the best trial was the point.

    The failure this catches is not a crash: the campaign runs, produces a best trial, and there is
    no rule that says whether that number was the point. The answer then gets decided afterwards, by
    whoever is reading, which is how a max-of-24 becomes a headline.
    """
...
def test_a_dimension_that_changes_the_data_needs_a_baseline():
    """Two trials that fit different label sets are not competing on the same problem.

    The one with more positives scores higher without being more predictable, so the search
    optimises the slice instead of the model.
    """
...
def test_a_declared_baseline_settles_the_comparability_question():
    warnings = goal_warnings(
        _goal(
            objective=ObjectiveSpec(metric="pr_auc", baseline_metric="baseline_pr_auc", mode="max"),
            space={
                "label_source": {
                    "type": "choice",
                    "values": ["work_orders", "manual"],
...
def test_asking_for_separation_without_measuring_spread_can_never_pass():
    ...
    A campaign built this way would report failure whatever it found.
    """
...
def test_asking_to_beat_a_baseline_that_is_not_declared_can_never_pass():
    warnings = goal_warnings(_goal(criteria=SuccessCriteria(require_beats_baseline=True)))
...
def test_a_goal_that_proves_nothing_can_be_refused_outright(tmp_path):
    """`--strict` is how CI and an agent driving the CLI stop instead of reading warnings."""
    from typer.testing import CliRunner
...
def test_variants_without_a_smoke_command_are_flagged():
    """Without a smoke command, nothing stands between an agent's edit and a round of trials.

    The harness records `smoke_ok: None` and runs it anyway, so a variant that cannot import costs
    the round and teaches nothing.
    """
    warnings = goal_warnings(_goal(variants=VariantSpec(enabled=True, editable_paths=["*.py"])))

    assert any("smoke_command" in w for w in warnings)


def test_variants_that_may_write_anywhere_are_flagged():
    """`editable_paths` empty means every file in the copied workload is writable.

    Including the evaluation the variant is scored by.
    """
    warnings = goal_warnings(
        _goal(variants=VariantSpec(enabled=True, smoke_command=[sys.executable, "-c", ""]))
    )
...
def test_counting_bootstrap_replicates_as_evidence_is_flagged():
    """`min_observations` asks how much evidence the score rests on.

    Under `bootstrap` the observations are resamples of one evaluation, so their count is how long
    the workload chose to resample -- a criterion that any workload can satisfy by looping more is
    not a criterion.
    """
    warnings = goal_warnings(
        _goal(
...
def test_bootstrap_satisfies_the_criteria_that_need_an_interval():
    """`require_separation` needs a standard error, and `bootstrap` produces one.

    Flagging it would push a goal back to the estimator with less information.
    """
    warnings = goal_warnings(
        _goal(
            objective=ObjectiveSpec(
