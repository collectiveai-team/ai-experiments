...
from tests.conftest import FakeMlflowClient, FakeMlflowModule
...
def test_a_transient_failure_still_syncs_when_the_server_comes_back(tmp_path):
    ...
    client = broken.last_client
    assert client is not None
    # Same object, mended in place: the run's artifact state has to survive the
    # "server came back" moment, which a fresh client would not carry over.
    client.__class__ = FakeMlflowClient
    healed.last_client = client
